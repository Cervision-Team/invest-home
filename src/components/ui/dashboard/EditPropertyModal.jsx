...
const EditPropertyModal = ({ editRow, onClose, onSave }) => {
if (!editRow) return null;

  const [activePropertyType, setActivePropertyType] = useState(editRow.propertyType || 'apartment');
  const [activeOfficeType, setActiveOfficeType] = useState(editRow.officeType || null);
  const [activeBuilding, setActiveBuilding] = useState(editRow.buildingType || null);
  const [activeRepaired, setActiveRepaired] = useState(editRow.repairStatus || null);
  const [isMortgaged, setIsMortgaged] = useState(editRow.isMortgaged || false);
  
  // Location state
  const [selectedAddress, setSelectedAddress] = useState(editRow.selectedAddress || '');
  const [searchQuery, setSearchQuery] = useState(editRow.searchQuery || '');
  ...
  const [images, setImages] = useState(editRow.images || []);
...
const fileInputRef = useRef(null);
  ...
  const [formData, setFormData] = useState({
    announcementId: editRow.announcementId || '',
    propertyType: editRow.propertyType || 'apartment',
    officeType: editRow.officeType || null,
    buildingType: editRow.buildingType || null,
    repairStatus: editRow.repairStatus || null,
    isMortgaged: editRow.isMortgaged || false,
    area: editRow.area || '',
    landArea: editRow.landArea || '',
    floor: editRow.floor || '',
    totalFloors: editRow.totalFloors || '',
    rooms: editRow.rooms || '',
    bathrooms: editRow.bathrooms || '',
    price: editRow.price || '',
    initialPayment: editRow.initialPayment || '',
    monthlyPayment: editRow.monthlyPayment || '',
    remainingMonths: editRow.remainingMonths || '',
    exit: editRow.exit || 'theres',
    mortgage: editRow.mortgage || 'yes',
    features: editRow.features || [],
    description: editRow.description || '',
    selectedCity: editRow.selectedCity || '',
    selectedDistrict: editRow.selectedDistrict || '',
    selectedSettlement: editRow.selectedSettlement || '',
    selectedAddress: editRow.selectedAddress || '',
    searchQuery: editRow.searchQuery || '',
    latitude: editRow.latitude || null,
    longitude: editRow.longitude || null
  });
console.log(editRow.selectedCity);
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        ...
        if (editRow.latitude && editRow.longitude) {
          addMarkerToMap(editRow.latitude, editRow.longitude, editRow.selectedAddress);
        }
    ...
    <div className="fixed inset-0 z-8000 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col scrollbar-custom-wrapper">
        ...
        <div className="flex items-center justify-between py-4.5 px-8 border-b border-gray-200">
          <h2 className="text-[24px]/[20px] font-medium text-[#0A0D14]">Düzəliş et</h2>
          <button
            onClick={onClose}
